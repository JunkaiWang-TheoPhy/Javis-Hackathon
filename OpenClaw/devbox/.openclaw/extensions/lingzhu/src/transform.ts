...
const IMAGE_FILE_SUFFIX_RE = /\.(?:png|jpe?g|gif|webp|avif|bmp|svg|tiff?|heic|heif|ico)(?:[?#].*)?$/iu;
...
function normalizeImageUrlCandidate(value: unknown): string | null {
  ...
  if (trimmed.startsWith("/") && IMAGE_FILE_SUFFIX_RE.test(trimmed)) {
...
function extractTextValue(message: LingzhuMessage): string {
  if (typeof message.text === "string" && message.text.trim()) {
    return message.text.trim();
  }

  if (typeof message.content === "string" && message.content.trim() && !normalizeImageUrlCandidate(message.content)) {
...
function collectImageUrls(message: LingzhuMessage): string[] {
  ...
  for (const attachment of message.attachments ?? []) {
    ...
    addUrl(attachment?.image_url);
    addUrl(attachment?.url);
    addUrl(attachment?.path);
    addUrl(attachment?.dataUrl);
    addUrl(attachment?.content);
  }
...
export function lingzhuToOpenAI(
  ...
  for (const msg of messages) {
    ...
    if (imageUrls.length > 0) {
      ...
      openaiMessages.push({ role, content: textValue });
    }
  }
