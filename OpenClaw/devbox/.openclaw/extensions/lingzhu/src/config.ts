...
const DEFAULT_CONFIG: Required<LingzhuConfig> = {
  ...
  memoryContextTimeoutMs: 1500,
  memoryContextWorkingLimit: 4,
  memoryContextFactLimit: 4,
};
...
export function resolveLingzhuConfig(raw: unknown): LingzhuConfig {
  ...
  const memoryContextTimeoutMs =
    typeof cfg.memoryContextTimeoutMs === "number" && Number.isFinite(cfg.memoryContextTimeoutMs)
      ? Math.max(500, Math.min(15000, Math.trunc(cfg.memoryContextTimeoutMs)))
      : DEFAULT_CONFIG.memoryContextTimeoutMs;
  const memoryContextWorkingLimit =
    typeof cfg.memoryContextWorkingLimit === "number" && Number.isFinite(cfg.memoryContextWorkingLimit)
      ? Math.max(1, Math.min(8, Math.trunc(cfg.memoryContextWorkingLimit)))
      : DEFAULT_CONFIG.memoryContextWorkingLimit;
  const memoryContextFactLimit =
    typeof cfg.memoryContextFactLimit === "number" && Number.isFinite(cfg.memoryContextFactLimit)
      ? Math.max(1, Math.min(8, Math.trunc(cfg.memoryContextFactLimit)))
      : DEFAULT_CONFIG.memoryContextFactLimit;
  ...
  return {
    ...
    memoryContextEnabled: cfg.memoryContextEnabled ?? DEFAULT_CONFIG.memoryContextEnabled,
    memoryContextUrl: typeof cfg.memoryContextUrl === "string" && cfg.memoryContextUrl.trim()
      ? cfg.memoryContextUrl.trim()
      : DEFAULT_CONFIG.memoryContextUrl,
    memoryContextAudience:
      cfg.memoryContextAudience === "direct" || cfg.memoryContextAudience === "shared"
        ? cfg.memoryContextAudience
        : DEFAULT_CONFIG.memoryContextAudience,
    memoryContextTimeoutMs,
    memoryContextWorkingLimit,
    memoryContextFactLimit,
  };
}
...
export const lingzhuConfigSchema = {
  ...
  properties: {
    ...
    memoryContextEnabled: { type: "boolean" as const },
    memoryContextUrl: { type: "string" as const },
    memoryContextAudience: { type: "string" as const, enum: ["auto", "direct", "shared"] },
    memoryContextTimeoutMs: { type: "number" as const, minimum: 500, maximum: 15000 },
    memoryContextWorkingLimit: { type: "number" as const, minimum: 1, maximum: 8 },
    memoryContextFactLimit: { type: "number" as const, minimum: 1, maximum: 8 },
  },
  ...
  uiHints: {
    ...
    memoryContextEnabled: {
      label: "启用记忆上下文",
      help: "在转发对话前调用本地 memory/context 接口，把工作记忆和长期事实注入 prompt",
    },
    memoryContextUrl: {
      label: "记忆上下文接口",
      help: "默认使用本机 3301 端口的 /v1/memory/context 路由",
    },
    memoryContextAudience: {
      label: "记忆受众",
      help: "auto 会根据会话模式自动判定 direct 或 shared",
    },
    memoryContextTimeoutMs: {
      label: "记忆超时 (ms)",
      help: "检索记忆上下文的超时时间，范围 500~15000",
    },
    memoryContextWorkingLimit: {
      label: "Working Memory 上限",
      help: "最多注入多少条近期工作记忆，范围 1~8",
    },
    memoryContextFactLimit: {
      label: "Long-Term Memory 上限",
      help: "最多注入多少条长期事实，范围 1~8",
    },
  },
};
