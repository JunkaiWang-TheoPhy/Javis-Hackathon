...
- 新增了 `plugins/openclaw-plugin-hue/`
  ...
  - `ha-control.home_execute_intent` 现在也可以按 `directAdapter` 元数据直连 Hue
- 新增了 `plugins/openclaw-plugin-google-home/`
  - 现在已经接上 shared auth gateway 的真实 OAuth callback / token flow
  - 仍然不声明 broad live Google Home control 已完成
...
- 新增了 `plugins/openclaw-plugin-smartthings/`
  - 现在除了 readiness，也有最小 live control：list / status / execute
- 新增了 `plugins/openclaw-plugin-alexa/`
  - 目前只做 readiness / account-linking checklist
- 新增了 `services/ecosystem-auth-gateway/`
  - 负责共享 OAuth callback、state 和 token persistence
- `apple home / homekit` 目前仍然只走 `Home Assistant` 路径，不提供伪直连 API
- `amazon alexa` 目前是 readiness-only，不声明已完成 live control

进展汇总见：

- `../../docs/openclaw-ha-ecosystem-progress-2026-03-15.md`
- `docs/home-ecosystem-support-matrix.md`
...
- `plugins/openclaw-plugin-google-home/`
  - Google Home / Nest auth gateway / OAuth 状态工具面
- `plugins/openclaw-plugin-lutron/`
  - Lutron readiness 和 local bridge session 工具面
- `plugins/openclaw-plugin-smartthings/`
  - SmartThings readiness + 最小 live control 工具面
...
- 如果要启用 `hue`、`google-home`、`lutron` 或 `smartthings` 直连 / readiness 插件：
...
- `openclaw-plugin-lutron`
- `openclaw-plugin-smartthings`
...
- Hue plugin tests
- Google Home / Nest auth-flow plugin tests
- Lutron readiness plugin tests
- Lutron local bridge session tests
- SmartThings minimal live-control plugin tests
- Alexa readiness plugin tests
- ecosystem auth gateway tests
- Rokid plugin helper tests
- Rokid bridge route tests
- bootstrap script coverage

截至 `2026-03-15`，`npm test` 在本仓库下是 `55/55` 通过。
...
- `plugins/openclaw-plugin-ha-control/src/direct-adapters.ts`
...
- `plugins/openclaw-plugin-lutron/src/index.ts`
- `plugins/openclaw-plugin-lutron/src/session.ts`
- `plugins/openclaw-plugin-smartthings/src/index.ts`
