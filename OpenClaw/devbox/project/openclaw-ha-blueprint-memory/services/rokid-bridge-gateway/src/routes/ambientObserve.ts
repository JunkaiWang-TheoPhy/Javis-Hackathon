...
import { buildAmbientObserveEvent } from "../memory/eventFactory.ts";
import type { MemoryLedger } from "../memory/memoryLedger.ts";

export async function handleAmbientObserveRequest(body: unknown, ledger?: MemoryLedger) {
  ...
  ledger?.record(buildAmbientObserveEvent(observation));
